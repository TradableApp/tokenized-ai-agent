...
import type { NewsSearchHit } from "@tradableapp/sense-ai-brain";
 ...
 * whitespace inside the extraction prompt. An earlier pass "tidied" both and the second silently
 * changed the bytes sent to the model, which is precisely the drift a shared Brain exists to
 * prevent. Tidy core if it wants tidying; this file follows.
 *
 * DO NOT RUN PRETTIER ON THIS FILE. It keeps core biome formatting (no trailing commas in
 ...
 * fail `prettier --check`, while every oracle-native file in this plugin passes it.
 *
 ...
 *      no news table at all. core line would compile, run, and query the wrong database. The
 ...
 *
 * THERE USED TO BE A THIRD. Two `as string` / `as number` assertions were needed because the
 * corresponding lines were TYPE ERRORS in core, tolerated there via `.tsc-baseline.txt` but fatal
 * to this plugin build. core fixed them at the source in #88 (CU-86d403h5a), so the assertions
 * are gone and the file is a pure copy again.
 */
...
export const getNewsDetailsAction: Action = {
  ...
  handler: async (
    ...
    // ORACLE DIVERGENCE 2 — core BROADCASTS_LIVE guard sits here and is deliberately absent.
    ...
    try {
      ...
      // The Brain's OWN return type, rather than a local restatement of it.
      //
      // This used to be an inline shape with an `[key: string]: unknown` index signature, and
      // both call sites cast onto it with `as typeof results`. That cast is what made the two
      // baselined type errors possible: `similarity` arrived as `unknown` through the index
      // signature (so `similarity * 100` was TS2362) and `publishedAt` was declared `string | undefined`
      // (so `new Date(...)` matched no overload, TS2769). Neither was a real runtime fault —
      // `published_at` is `NOT NULL` in the schema — but the compiler could not know that, and
      // the cast meant it could not check anything else at this boundary either.
      //
      // Using `NewsSearchHit` immediately caught something the cast had been hiding: `metadata`
      // is returned by the title-lookup strategy and omitted by both hybrid paths, and the type
      // did not declare it at all. Fixed upstream in sense-ai-brain rather than widened here.
      let results: NewsSearchHit[] = [];
      ...
      if (intent.type === "specific" && intent.targetTitles && intent.targetTitles.length > 0) {
        // `[GetNewsDetails]` prefix added for consistency: every other log in this action carries
        // it, and this line did not. It also makes the tag usable as a filter — `createBrainContext`
        // routes the Brain's `ctx.logger.info` to `elizaLogger.info`, so a test spying on the
        // logger sees Brain lines too, and needs a reliable way to keep only the action's own.
        elizaLogger.info(
          `[GetNewsDetails] Strategy: Batch Title Lookup -> ${intent.targetTitles.length} Titles`
        );
      ...
      // =================================================================
      const formattedData = results.map((item: NewsSearchHit) => ({
        title: item.title,
        url: item.url,
        ...
        //
        // `!= null` rather than truthiness, now that the type says `number | undefined`. The old
        // gate treated a similarity of `0` as "unranked" and rendered "Exact", which is the
        // opposite of the truth — 0 means ranked and maximally dissimilar.
        //
        // Behaviour is unchanged, because 0 cannot currently reach here. All three Brain paths
        // were checked: the title lookup omits `similarity` entirely, the text fallback sets a
        // constant `1` sentinel, and the vector path filters on `similarity > floor` where the
        // lowest floor is 0.35. So this is not a fix for a live symptom — it removes a latent
        // mislabel that would surface the day any of those three change.
        similarity: item.similarity != null ? Math.round(item.similarity * 100) : "Exact",
      }));
