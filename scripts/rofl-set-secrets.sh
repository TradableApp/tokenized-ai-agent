...
awk '
  ...
  in_env { next }
  { print }
' "$COMPOSE_TEMPLATE" > "${COMPOSE_TEMPLATE}.new"
# Drift guard (same class as PART 4's): if `environment:` is ever removed or renamed,
# the awk matches nothing, copies the file through verbatim and exits 0 — so the
# secrets block would silently NOT be regenerated and the deployment would run on a
# stale env. Assert the marker survived, and that we actually emitted our own block.
if ! grep -q 'environment:' "${COMPOSE_TEMPLATE}.new" ||
  ! grep -q 'AUTO-GENERATED by scripts/rofl-set-secrets.sh' "${COMPOSE_TEMPLATE}.new"; then
  echo "❌ compose rewrite failed: no 'environment:' block was regenerated in"
  echo "   $COMPOSE_TEMPLATE. The key may have been renamed/removed — fix the template"
  echo "   (or the awk) rather than deploying a stale environment block."
